...
class MCMCAlgorithm(ABC):
    # ----------------------------------------------------------------------------------------------
    def __init__(self, settings: AlgorithmSettings, model: model.MCMCModel):
    ...
    # ----------------------------------------------------------------------------------------------
    def compute_step(self, current_state: np.ndarray):
    ...
    # ----------------------------------------------------------------------------------------------
    def _perform_accept_reject(self, current_state: np.ndarray, proposal: np.ndarray):
    ...
    # ----------------------------------------------------------------------------------------------
    @staticmethod
    def _choose_args_to_cache(computed_args: dict[str, tuple], accepted: bool):
    ...
    # ----------------------------------------------------------------------------------------------
    def _cache_args(self, computed_args: dict[str, tuple]):
        for arg in computed_args:
            self._cached_args[arg] = computed_args[arg]

    # ----------------------------------------------------------------------------------------------
    @abstractmethod
    def _create_proposal(self, state: np.ndarray):
        pass

    # ----------------------------------------------------------------------------------------------
    @abstractmethod
    def _evaluate_acceptance_probability(self, current_state: np.ndarray, proposal: np.ndarray):
        pass
...
class pCNAlgorithm(MCMCAlgorithm):
    ...
    # ----------------------------------------------------------------------------------------------
    def create_proposal(self, state: np.ndarray):
        ...
        proposal = self._step_width * state + np.sqrt(1 - self._step_width**2) * random_increment
        computed_args = {}
